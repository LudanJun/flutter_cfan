...
import 'package:cfan_flutter/page/tabbars/cfan/model/cfan_community_my_fans.dart';
...
class CfanCommunityFansPage extends StatefulWidget {
  //传入社群id
  final int community_id;
  const CfanCommunityFansPage({super.key, required this.community_id});

  @override
  State<CfanCommunityFansPage> createState() => _CfanCommunityFansPageState();
}

class _CfanCommunityFansPageState extends State<CfanCommunityFansPage> {
  //刷新控制器
  ...
  List<CfanCommunityMyFansItemModel> _fansItemModelList =
  ...
  //获取自己在社群中的粉丝信息
  getCommunityMyFansData(int page) {
    if (_flag && _hasData) {
      _flag = false;
      _cfanCommunityHomeProvider.userPostsCommunityMyFans(
        widget.community_id,
        page,
        onSuccess: (data) {
          if (data['code'] == 200) {
            var tempData = CfanCommunityMyFansModel.fromJson(data);
            setState(() {
              _fansItemModelList.addAll(tempData.data ?? []);
              _page++;
              _flag = true;
            });
            //判断有没有数据
    ...
    return Scaffold(
      ...
      body: EasyRefresh(
        ...
        child: ListView.builder(
          itemCount: _fansItemModelList.length,
          itemBuilder: (context, index) {
            var itemModel = _fansItemModelList[index];
            return Container(
